...
#  * the name alone is not enough either: a DIRECTORY named e.g. "dir.protos" would satisfy the
#    guard below and hand protoc a directory as an input file.
...
#...and the number is ACTED ON, not just printed, like every other target's. The mod.rs check
#above already covers the plugin-did-not-run case (mod.rs is itself a *.rs under this directory,
#so a surviving run counts at least one), which leaves this as the guard that keeps the
#invariant "a zero stub count is an error" true for all eleven targets rather than ten.
if [ "$STUB_FILES_CNT" -lt 1 ]; then
    echo "ERROR: protoc reported success but produced no '*.rs' sources in '$CRATE_API_DIR' - there is nothing to build a library from - exiting" >&2
    exit 1
fi
