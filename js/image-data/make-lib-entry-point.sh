...
PUBLIC_API_FILE=$TEMP_SRC_DIRECTORY/public-api.js

if [ ! -f $PUBLIC_API_FILE ]; then
    echo "No public-api.js specified in source directory -> copying default file"
    cp $DEFAULT_FILES_DIR/public-api.js $PUBLIC_API_FILE
    ...
    find api -iname "*.js" -exec bash -c 'printf "$PREFIX./%s$POSTFIX\n" "${@%.*}"' _ {} + >> $PUBLIC_API_FILE
fi