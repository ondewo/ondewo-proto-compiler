...
#nothing from, and the wipe above has already deleted the previous run's stubs - so an empty
#api/ would be copied over a working client module and break every consumer's build.
if [ "$STUB_FILES_CNT" -lt 1 ]; then
    echo "ERROR: protoc reported success but produced no '*.go' sources in '$STUBS_TARGET_DIR' - there is nothing to build a library from - exiting" >&2
    exit 1
fi
