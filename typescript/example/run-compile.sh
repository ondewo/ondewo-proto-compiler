...
if [ -z "$1" ]; then
    ...
    docker run -v "$FILEDIRECTORY":/input-volume -v "$FILEDIRECTORY/lib":/output-volume ondewo-typescript-proto-compiler protos library
else
...
#docker run -v $FILEDIRECTORY:/input-volume ondewo-angular-proto-compiler
