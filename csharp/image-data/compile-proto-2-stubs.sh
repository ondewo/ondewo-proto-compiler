...
#    no-protos guard below and then be handed to protoc, which fails with "Is a directory".
...
#nothing from, and an empty api/ then produces a .nupkg with no types in it that only fails at
#the consumer.
if [ "$STUB_FILES_CNT" -lt 1 ]; then
    echo "ERROR: protoc reported success but produced no '*.cs' sources in '$STUBS_TARGET_DIR' - there is nothing to build a library from - exiting" >&2
    exit 1
fi
