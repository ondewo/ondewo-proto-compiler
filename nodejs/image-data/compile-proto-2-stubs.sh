...
#  * the name alone is not enough either: a DIRECTORY whose name ends in .proto would satisfy the
#    no-protos guard below and then be handed to protoc as an input file, which aborts with its
#    own confusing error instead of this script's "No proto files were found" one.
...
#and a directory whose name ends in .ts would inflate it
...
#into the client repo, whose previously generated api/ the orchestrator has already wiped.
